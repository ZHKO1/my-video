...
from my_video.core.asr.text_diff import normalize_whitespace, render_inline_diff, rewrite_segments_with_timestamps
...
def test_strict_diff_detects_case_changes() -> None:
    assert render_inline_diff("Hello world", "hello world", mode="strict", display="reference") == (
        "【Hello/hello】 world"
    )


def test_strict_diff_detects_punctuation_changes() -> None:
    assert render_inline_diff("USB", "USB.", mode="strict", display="reference") == "【USB/USB.】"
...
def test_rewrite_segments_with_timestamps_from_origin_text() -> None:
    ...
    rewritten = rewrite_segments_with_timestamps(segments, "HELLO brave world!", mode="strict")
