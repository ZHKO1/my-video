...
from my_video.cli.config import get_toml_str, load_toml_config
...
def _get_source_language() -> str:
    return get_toml_str(_load_config(), "transcribe.whisperx.language", default="auto") or "auto"


def _get_target_language() -> str:
    return get_toml_str(_load_config(), "subtitle.target_language", default="zh") or "zh"


## ================================================================
# @ step4_splitbymeaning.py
def get_split_prompt(sentence, num_parts=2, word_limit=20):
    language = _get_source_language()
    split_prompt = f"""
## Role
...
def get_summary_prompt(source_content, custom_terms_json=None):
    src_lang = _get_source_language()
    tgt_lang = _get_target_language()
...
def get_prompt_faithfulness(lines, shared_prompt):
    target_language = _get_target_language()
    ...
    src_language = _get_source_language()
...
def get_align_prompt(src_sub, tr_sub, src_part):
    targ_lang = _get_target_language()
    src_lang = _get_source_language()
